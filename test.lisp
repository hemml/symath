...
(deftest expand-mul ((symath) (equal-expr))
...
(symath::defun-stable-expr cc-test (e)
  (symath::chain-func-rec
    (symath::extract-nums
     symath::collect-common-nums
     symath::collect-exprs
     symath::collect-common)
    e))

(deftest collect-common ((symath) (equal-expr equal-expr-1 extract-nums collect-expt collect-exprs))
  ...
  (assert-true (symath::equal-expr (symath::collect-common '(+ (* (+ x 10) y) (* (+ x 10) z) a))
                                   '(+ (* (+ x 10) (+ y z)) a)))
  (assert-true (symath::equal-expr (cc-test '(+ (* (+ x 10) (+ y 20)) (+ y 20) a))
                                   '(+ (* (+ y 20) (+ x 11)) a)))
  ...
  (assert-true (symath::equal-expr (symath::collect-common '(+ (* (expt x 2) a) (* (expt x 2) b) (* y a) (* y b)))
                                   '(* (+ (expt x 2) y) (+ a b))))
  (assert-true (symath::equal-expr (cc-test '(+ (expt x 2) (expt x 3) (expt x 4)))
                                   '(* (expt x 2) (+ 1 (* x (+ x 1))))))
  (assert-true (symath::equal-expr (cc-test '(+ (expt x 2) (* a (expt x 3)) (* (expt x 4) b)))
                                   '(* (expt x 2) (+ 1 (* x (+ a (* x b)))))))
  (assert-true (symath::equal-expr (cc-test '(+ (* x a q) (* x b q) (* y a q) (* y b q)))
                                   '(* q (+ x y) (+ a b))))
