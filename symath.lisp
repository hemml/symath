                                    ...
                                    (let* ((term (car terms))
                                      ...
                                      (multiple-value-bind (q r) (floor nt num)
                                        (multiple-value-bind (t1 t2) (filter-terms num (cdr terms))
                                          (if (= r 0)
                                              (values (cons (cond ((numberp term) q)
                                                                  ((listp term)
                                                                   (if (= q 1)
                                                                       (if (> (length term) 3)
                                                                           `(* ,@(cddr term))
...
(defun simplify-expr2 (e) ;; normalize -> simplify -> denormalize
  (with-templates (((diff @ $) => 0)
                   ((diff _1 _1) => 1)
                   ((diff (+ &rest args) _1) =>
                    (cons '+ (mapcar #'simplify-expr2 (mapcar (compose (curry #'cons 'diff) (rcurry #'list _1)) args))))
                   ((diff (* &rest args) _1) =>
                    `(+ (* (diff ,(car args) ,_1)
                           ,@(cdr args))
                        (* ,(car args)
                           ,(simplify-expr2 `(diff ,(if (cddr args)
                                                        `(* ,@(cdr args))
                                                        (cadr args))
                                                   ,_1)))))
                   ((diff (exp $1) _2) => `(* (exp ,$1) ,(simplify-expr2 `(diff ,$1 ,_2))))
                   ((diff (expt $1 $2) _3) => (simplify-expr2 `(diff (exp (* ,$2 (log ,$1))) ,_3)))
                   ((diff (log $1) _2) => `(/ ,(simplify-expr2 `(diff ,$1 ,_2)) ,$1))
                   ((diff (sin $1) _2) => `(* (cos ,$1) ,(simplify-expr2 `(diff ,$1 ,_2))))
                   ((diff (cos $1) _2) => `(* -1 (sin ,$1) ,(simplify-expr2 `(diff ,$1 ,_2))))
                   ((diff (tan $1) _2) => `(/ ,(simplify-expr2 `(diff ,$1 ,_2)) (expt (cos ,$1) 2)))
                   ((diff (ctan $1) _2) => `(* -1 (/ ,(simplify-expr2 `(diff ,$1 ,_2)) (expt (sin ,$1) 2))))
                   ((diff (asin $1) _2) => `(/ ,(simplify-expr2 `(diff ,$1 ,_2)) (sqrt (- 1 (expt $1 2)))))
                   ((diff (acos $1) _2) => `(* -1 (/ ,(simplify-expr2 `(diff ,$1 ,_2)) (sqrt (- 1 (expt $1 2))))))
                   ((diff (atan $1) _2) => `(/ ,(simplify-expr2 `(diff ,$1 ,_2)) (+ 1 (expt $1 2))))
